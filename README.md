...
- **目標到達の知らせ**（音 / バイブ・既定オフ） — 作図中で画面を見ていないときに、工程の目標時間に達したことを手を止めずに知れる。端末が対応している手段だけが選択肢に出る（iPhone / iPad は Vibration API 非対応のため音のみ。消音にしていると音は鳴らない）
- **画面スリープの防止**（対応ブラウザのみ）
...
- **累計カウンターと直近6ヶ月のヒートマップ** — 総セッション数 / 練習した日数 / 総練習時間と、日ごとの練習量のカレンダー。連続日数ではなく積み上げた量で見せるので、1日休んでも数字は減りません（フィルタに連動せず常に全記録が対象）
